...
const TableConetext = createContext({
  searches: {},
  setSearch: () => {},
});
...
export const filterByColumnSearches = (rows, searched, filterFns = {}) => {
...
const Table = ({
  ...
  return (
    <TableConetext.Provider value={{ searches, setSearch }}>
      <div
...
Table.Header = ({ children, className = "" }) => {
  ...
  const hasSearchRow = Children.toArray(children).some(
    (child) =>
      isValidElement(child) && child.props.searchable && child.props.dataKey,
  );
              ...
              >
                <div className="relative group">
                  ...
                  <input
                    ...
                    placeholder="Search.."
                    className="w-full pl-7 pr-7 py-1.5 text-xs font-momo rounded-lg bg-slate-800/80 border text-slate-100 placeholder-slate-600 outline-none transition-all duration-150 border-slate-700 focus:border-emerald-500/70 focus:ring-2 focus:ring-emerald-500/20 hover:border-slate-600"
                  />
                  ...
                  {searches[dataKey] && (
                    <button
                      ...
                      aria-label={`Clear search for ${dataKey}`}
                    >
                      <svg
                        width="10 "
                        height="10"
...
Table.HeaderCell = ({
  ...
  return (
    <th
      className={`px-5 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest border-r border-slate-700/50 last:border-r-0 transition-colors duration-150 ${isActive ? "text-emerald-400 bg-emerald-950/30" : "text-slate-400"} ${className}`}
      style={{ width: width || "auto" }}
      {...props}
    >
      <div className="flex items-center gap-2">
        ...
        {searchable && (
          <span
            className={`inline-flex items-center gap-1 text-[9px] font-semibold normal-case tracking-normal px-1.5 py-0.5 rounded-full border transition-all duration-150 ${isActive ? "border-emrald-500/60 text-emerald-400 bg-emerald-500/10" : "border-slate-700 text-slate-600 bg-slate-800/50"}`}
          >
            {isActive ? (
              <>
                <svg
        ...
        <td colSpan={999} className="px-6 py-14 text-center">
          <div className="flex flex-col items-center gap-3 text-slate-500">
            ...
            <span className="text-xs text-slate-600">
              Try broading your search Terms.
            </span>
          </div>
        </td>
      </tr>
...
<tr className={`${hover?"hover:bg-slate-800/40":""}
...
Table.Cell=({children,className="",width,...props})=>(
  <td className={`px-5 py-4 whitespace-normal text-slate-300 text-sm border-r border-slate-800/60 last:border-r-0 ${className}`}
  style={{width:width ||"auto"}}
  {...props}>
    {children}
  </td>
